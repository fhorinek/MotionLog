...
uint8_t ram_bt_link_partner[6];
uint8_t ram_bt_link_key[16];
uint8_t ram_btle_mac[6];
uint8_t ee_bt_link_partner[6];
uint8_t ee_bt_link_key[16];
uint8_t ee_btle_mac[6];
	...
	switch (op_code)
	{
		...
		case(0x47)://TCU_MNG_CONNECTION_STATUS_EVENT
			...
			if (this->parser_buffer[14] == 0x03)
			{
				...
				for (uint8_t i = 0; i < 16; i++)
					ram_bt_link_key[i] = this->parser_buffer[15 + i];

				eeprom_busy_wait();
				eeprom_update_block((void *)&ram_bt_link_key, &ee_bt_link_key, 16);

				for (uint8_t i = 0; i < 6; i++)
				{
					ram_bt_link_partner[i] = this->parser_buffer[8 + i];
					DEBUG_BT("%02X ", ram_bt_link_partner[i]);
				}
				DEBUG_BT("\n");

				eeprom_busy_wait();
				eeprom_update_block((void *)&ram_bt_link_partner, &ee_bt_link_partner, 6);
			...
			if (status == 0x87)
			{
				...
				memset((void *)ram_bt_link_partner, 0, sizeof(ram_bt_link_partner));
				memset((void *)ram_bt_link_key, 0, sizeof(ram_bt_link_key));
	...
	switch(op_code)
	{
		...
		case(0x48): // TCU_SPP_DATA_RECEIVE_EVENT
			...
			for (i = 0; i < tmp2.uint16; i++)
				bt_irqh(BT_IRQ_DATA, &this->parser_buffer[9 + i]);
	...
	switch (op_code)
	{
		...
		case(0x54): //TCU_MNG_LE_GEN_RESOLVABLE_BDADDR_RESP
			...
			if (status == 0x00)
			{
				...
				memcpy((void *)ram_btle_mac, &this->parser_buffer[8], 6);
				eeprom_busy_wait();
				eeprom_update_block((void *)&ram_btle_mac, &ee_btle_mac, 6);
			}
			else
				PAN1026_ERROR;
		break;
	...
	switch (op_code)
	{
		...
		case(0xC3): //TCU_LE_GATT_SER_WRITE_CHAR_VAL_EVENT
			...
			for (uint16_t i = 0; i < len - 4; i++)
			{
				...
				bt_irqh(BT_IRQ_DATA, &this->parser_buffer[11 + i]);
			}
			DEBUG_BT("\n");
...
const uint8_t PROGMEM manufacturer_name[] = {'N', 'a', 'n', 'o', 'd', 'e', 's'};
const uint8_t PROGMEM device_name_spp[] = {'B', 'i', 'o', 'p', 'r', 'o', 'b', 'e', 'S', 'P', 'P'};
const uint8_t PROGMEM device_name_ble[] = {'B', 'i', 'o', 'p', 'r', 'o', 'b', 'e', 'B', 'L', 'E'};
...
void pan1026::Step()
{
	...
	if (this->next_cmd != pan_cmd_none)
	{
		...
		switch(this->next_cmd)
		{
			...
			case(pan_cmd_accept_connection):
				...
				for (uint8_t i = 0; i < 6; i++)
				{
					if (ram_bt_link_partner[i] != this->client_mac_address[i])
					{
						use_link_key = 0;
						break;
					}
				...
				if (use_link_key)
				{
					...
					for (uint8_t i = 0; i < 16; i++)
					{
						this->StreamWrite(ram_bt_link_key[i]);
						DEBUG_BT("%02X ", ram_bt_link_key[i]);
					}
					DEBUG_BT("\n");
				}
				else
	...
	//			RAW(TCU_MNG_SSP_SET_REQ_HCI_User_Confirmation_Negative_Reply);
			...
			case(pan_cmd_le_set_random_address):
				...
				for (uint8_t i = 0; i < 6; i++)
					this->StreamWrite(ram_btle_mac[i]);
			break;
